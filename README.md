...
燧石启动器（Chert / MCLCS，Chert Minecraft Launcher / Minecraft Launcher CSharp）是一个用 C# / WPF 实现的 Minecraft 启动器，覆盖版本安装、启动、崩溃修复、下载、Mod 管理与工具箱等。本项目与 [Chert-Linux](https://cnb.cool/RLRS-Studio/Chert-Linux) 共享核心（`Chert.Core`），两端功能持续对齐。
...
| **v2.5.5（最新）** | [CNB Releases](https://cnb.cool/RLRS-Studio/Chert-WPF/-/releases) | 自包含免运行时，解压即用 |
